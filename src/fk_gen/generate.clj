...
(defn ->sql
  ...
  (let [db-info->connection-uri (fn [{:keys [subprotocol subname user schema password]}]
                                  {:connection-uri (str "jdbc:" subprotocol ":" subname "?user=" user "&password=" password)
                                   :schema schema})
        gen-values (fn [table] (transform [MAP-KEYS] #(keyword (name %)) (last (gen/sample (s/gen (keyword (str "table/" (name table))) 30)))))
        table-graph->insert-stmt-plan (partial ->insert (fn [table graph] [(merge (gen-values table) (->select-any table graph))]))]
